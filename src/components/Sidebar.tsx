...
export default function Sidebar() {
  ...
  return (
    <aside className={`fixed top-0 left-0 h-full z-40 flex flex-col bg-[#FCFCFD] dark:bg-[#09090B] border-r border-[#E2E8F0] dark:border-[#ffffff08] transition-all duration-300 ${sidebarOpen ? 'w-52' : 'w-16'}`}>
      {/* Logo */}
      <div className="flex items-center justify-between px-4 h-14 border-b border-[#E2E8F0] dark:border-[#ffffff08]">
        {sidebarOpen && (
          <span className="font-bold text-base text-[#2563EB] dark:text-blue-400 tracking-tight">
            Placement OS
          </span>
        )}
        <button onClick={toggleSidebar} className="p-1.5 rounded-[10px] text-[#94A3B8] hover:text-[#0F172A] hover:bg-[#F1F5F9] dark:hover:text-zinc-300 dark:hover:bg-white/5 transition-colors" aria-label={sidebarOpen ? 'Collapse' : 'Expand'}>
      ...
      {/* User area */}
      <div className="px-3 py-3 border-b border-[#E2E8F0] dark:border-[#ffffff08]">
        {sidebarOpen ? (
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-[10px] bg-[#2563EB] flex items-center justify-center text-sm font-bold text-white flex-shrink-0">
              {displayName.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase() || 'U'}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-xs font-semibold text-[#0F172A] dark:text-zinc-200">{displayName}</p>
              <p className="text-[10px] text-[#2563EB] dark:text-blue-400">Level {level}</p>
            </div>
          </div>
        ) : (
          <div className="flex justify-center">
            <div className="w-8 h-8 rounded-[10px] bg-[#2563EB] flex items-center justify-center text-xs font-bold text-white">
              {displayName.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase() || 'U'}
            </div>
          </div>
        )}
        {sidebarOpen && (
          <div className="mt-2.5 space-y-1">
            <div className="flex items-center justify-between text-[10px] text-[#64748B] dark:text-zinc-400">
              <span>XP</span>
              <span className="font-mono">{xpInLevel}/{XP_PER_LEVEL}</span>
            </div>
            <div className="h-1.5 bg-[#E2E8F0] dark:bg-zinc-800 rounded-full overflow-hidden">
              <div className="h-full rounded-full bg-[#2563EB]" style={{ width: `${xpProgress}%` }} />
            </div>
          </div>
        )}
      </div>
      ...
      <nav className="flex-1 overflow-y-auto py-3">
        {navGroups.map((group) => (
          <div key={group.label} className="mb-3">
            {sidebarOpen && (
              <p className="px-5 text-[10px] font-semibold uppercase tracking-widest text-[#64748B] dark:text-zinc-500 mb-1.5">{group.label}</p>
            )}
            {group.items.map((item) => {
              ...
              return (
                <NavLink
                  ...
                  className={`sidebar-item relative ${isActive ? 'sidebar-item-active' : isDisabled ? 'text-[#94A3B8] cursor-not-allowed' : 'sidebar-item-inactive'}`}
                >
                  {isActive && (
                    <motion.div layoutId="sidebar-indicator" className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-5 bg-[#2563EB] rounded-full" transition={{ type: 'spring', stiffness: 300, damping: 30 }} />
                  )}
                  ...
                  {sidebarOpen && isDisabled && <span className="text-[9px] px-1 py-0.5 rounded bg-[#F1F5F9] dark:bg-zinc-800 text-[#94A3B8] dark:text-zinc-500 ml-auto uppercase">Soon</span>}
                </NavLink>
              )
            })}
          </div>
